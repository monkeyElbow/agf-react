  ...
  (Array.isArray(blocks) ? blocks : []).forEach((block, index) => {
    ...
    (Array.isArray(block?.editableFields) ? block.editableFields : []).forEach((field) => {
      ...
      if (field?.type === 'route_link') {
        ...
        ['legacyHrefFieldId', 'routeRefFieldId', 'linkJsonFieldId', 'openInNewWindowFieldId'].forEach((metadataField) => {
          if (String(field?.[metadataField] || '').trim()) {
            findings.push(createFinding('canonical-link-legacy-metadata-persisted', 'Route-link editable fields must not persist legacy split-link metadata.', {
              ...location,
              field: fieldId,
              metadataField,
            }));
