...
describe('retirement rollovers review polish guardrail', () => {
  it('keeps the start the process panel on branded counters with softened address typography', () => {
    ...
    expect(contentSource).toContain("className: 'retirement-rollovers-native-process'");
    expect(contentSource).toContain("anchorId: 'start-the-process'");
    expect(contentSource).toContain("title: 'Start the process'");
    expect(contentSource).toContain("'Download and complete the Rollover/Transfer form below.'");
    expect(contentSource).toContain("'Return the completed form, along with the most recent statement(s) from the other account(s) to the address below.'");
    expect(contentSource).toContain("'A confirmation letter will be sent to you when your rollover or transfer is complete.'");
    expect(contentSource).not.toContain("'1) Download and complete the Rollover/Transfer form below.'");
    expect(contentSource).not.toContain("'2) Return the completed form, along with the most recent statement(s) from the other account(s) to the address below.'");
    expect(contentSource).not.toContain("'3) A confirmation letter will be sent to you when your rollover or transfer is complete.'");
    expect(contentSource).toContain("title: 'AGFinancial'");
    expect(contentSource).toContain("lines: ['PO Box 2515', 'Springfield MO 65801']");
    ...
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-process > .ag-panel-rail > p::before {');
    ...
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-process > .ag-panel-rail > p:nth-of-type(1)::before {');
    expect(cssSource).toContain('color: var(--ag-color-atlantean);');
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-process > .ag-panel-rail > p:nth-of-type(2)::before {');
    expect(cssSource).toContain('color: var(--ag-color-mango);');
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-process > .ag-panel-rail > p:nth-of-type(3)::before {');
    expect(cssSource).toContain('color: var(--ag-color-sandstone);');
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-process > .ag-panel-rail > p:nth-of-type(3) {');
  ...
  it('keeps the follow-up section on the managed request-form path instead of the legacy inline cta shell', () => {
    ...
    const rolloversContentSlice = contentSource.slice(
      contentSource.indexOf("'/services/retirement/rollovers': {"),
      contentSource.indexOf('};\n\nfunction serviceChildContent'),
    );

    expect(rolloversContentSlice).toContain("className: 'retirement-rollovers-native-request'");
    expect(rolloversContentSlice).not.toContain("className: 'retirement-rollovers-native-cta retirement-child-native-cta'");
    expect(rolloversContentSlice).not.toContain("submitLabel: 'Follow-up with me'");
    ...
    expect(blueprintSource).toContain("targetSectionKey: 'class:retirement-rollovers-native-request'");
    expect(blueprintSource).toContain("targetSectionClassName: 'retirement-rollovers-native-request'");
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .service-native-section.retirement-rollovers-native-request.native-dynamic-request {');
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-request.native-dynamic-request .dynamic-request-layout {');
    ...
    expect(cssSource).toContain('.native-info-page--retirement-rollovers .retirement-rollovers-native-request.native-dynamic-request > .ag-panel-rail {');
