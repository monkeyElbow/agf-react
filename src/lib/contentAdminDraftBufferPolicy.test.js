...
describe('content admin draft buffer policy', () => {
  it('keeps text edits buffered and discrete presentation choices immediate', () => {
    ...
    expect(shouldBufferLocalBlockSetting('titleFontWeight', 900)).toBe(false);
    expect(shouldBufferLocalBlockSetting('subtitleFontWeight', 700)).toBe(false);
    ...
    expect(getSharedBlockDraftSyncDelay('titleFontWeight', 900)).toBe(SHARED_BLOCK_DRAFT_SYNC_DISCRETE_DELAY_MS);
    expect(getSharedBlockDraftSyncDelay('subtitleFontWeight', 700)).toBe(SHARED_BLOCK_DRAFT_SYNC_DISCRETE_DELAY_MS);
  });
