...
describe('impact proof story layout guardrail', () => {
  it('keeps the desktop proof copy wider while preserving the stacked mobile fallbacks', () => {
    ...
    expect(cssSource).toContain('--impact-proof-dark-stop-3: 72%;');
    ...
    expect(cssSource).toContain('font-size: clamp(110px, 8.8vw, 136px);');
    expect(cssSource).toContain('font-weight: 700;');
    expect(cssSource).toContain('font-synthesis: none;');
    expect(cssSource).toContain('letter-spacing: -0.038em;');
    ...
    expect(cssSource).toContain('font-size: clamp(52px, 12vw, 72px);');
