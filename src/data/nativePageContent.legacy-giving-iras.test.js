...
describe('legacy giving and IRA native page content', () => {
  it('keeps the legacy giving copy and endowments/generosity cleanup in the native seed', () => {
    ...
    const charitableTrustsCrt = charitableTrustsContent?.sections?.find((section) => section?.className === 'legacy-child-native-trusts-crt');
    ...
    const charitableTrustsCrtIndex = charitableTrustsContent?.sections?.findIndex((section) => section?.className === 'legacy-child-native-trusts-crt');
    ...
    expect(charitableTrustsCrt?.anchorId).toBe('crt');
    expect(charitableTrustsCrt?.title).toBe('Charitable Remainder Trust');
    expect(charitableTrustsCrt?.body?.[0]).toContain('The trust pays you (and your spouse, if married) income for life.');
    expect(charitableTrustsTypes?.cards).toHaveLength(2);
    expect(charitableTrustsTypes?.cards?.[0]?.title).toBe('Charitable Remainder Unitrust (CRUT)');
    ...
    expect(charitableTrustsCrtIndex).toBeGreaterThan(-1);
    expect(charitableTrustsTypesIndex).toBeGreaterThan(-1);
    expect(charitableTrustsTypesIndex).toBeGreaterThan(charitableTrustsCrtIndex);
