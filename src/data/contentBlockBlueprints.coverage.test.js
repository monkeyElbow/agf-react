...
describe('source-default content block blueprint coverage', () => {
  ...
  it('seeds the 403(b) loan section with explicit semantic blocks instead of native-only sections', () => {
    ...
    const loanDetailsBlock = blocks.find((block) => block?.id === 'loan_details' && block?.kind === 'card_grid');
    ...
    expect(loanDetailsBlock?.presetId).toBe('loan-details');
    expect(loanDetailsBlock?.templateId).toBe('card_grid');
    ...
    expect(loanDetailsBlock?.settings?.title).toBe('403(b) Plan Loans');
    expect(String(loanDetailsBlock?.settings?.card1Body || '')).toContain('The requested 403(b) loan amount cannot be less than $1,500');
    expect(String(loanDetailsBlock?.settings?.card1Body || '')).toContain('retirement-403b-loan-detail-card');
    expect(String(loanDetailsBlock?.settings?.card1Body || '')).toContain('retirement-403b-loan-followup');
    expect(String(loanDetailsBlock?.settings?.card1Body || '')).toContain('retirement-403b-loan-fineprint');
    expect(String(loanDetailsBlock?.settings?.card1Body || '')).toContain('Contact your AGFinancial retirement consultant for more information.');
